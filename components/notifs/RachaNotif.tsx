...
export function RachaNotif({ show, racha, onClose }: RachaNotifProps) {
  ...
  const startWiggle = () => {
    ...
    wiggleLoop.current = Animated.loop(
      Animated.sequence([
        Animated.parallel([
          Animated.timing(fireRot,    { toValue:  1,    duration: 550, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
          Animated.timing(fireWiggle, { toValue:  1.04, duration: 550, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
          Animated.timing(haloScale,  { toValue:  1.04, duration: 550, useNativeDriver: true }),
        ]),
        Animated.parallel([
          Animated.timing(fireRot,    { toValue: -1,    duration: 600, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
          Animated.timing(fireWiggle, { toValue:  0.97, duration: 600, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
          Animated.timing(haloScale,  { toValue:  0.97, duration: 600, useNativeDriver: true }),
        ]),
        Animated.parallel([
          Animated.timing(fireRot,    { toValue:  0.4,  duration: 500, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
          Animated.timing(fireWiggle, { toValue:  1.02, duration: 500, useNativeDriver: true }),
          Animated.timing(haloScale,  { toValue:  1.02, duration: 500, useNativeDriver: true }),
        ]),
        Animated.parallel([
          Animated.timing(fireRot,    { toValue:  0,    duration: 500, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
          Animated.timing(fireWiggle, { toValue:  1,    duration: 500, useNativeDriver: true }),
          Animated.timing(haloScale,  { toValue:  1,    duration: 500, useNativeDriver: true }),
        ]),
      ])
    );
  ...
  useEffect(() => {
    if (!show) {
      ...
      Animated.parallel([
        ...
        Animated.timing(scale,   { toValue: reduceMotion ? 1 : 0.6, duration: 200, useNativeDriver: true }),
      ]).start();
      return;
    }
    ...
    // 2. Card bounce
    Animated.spring(scale,      { toValue: 1, tension: 170, friction: 8, useNativeDriver: true }).start();
    Animated.spring(translateY, { toValue: 0, tension: 140, friction: 9, useNativeDriver: true }).start();
  ...
  const visibleMilestones = MILESTONES.filter((m) => m <= racha + 20).slice(0, 5);
