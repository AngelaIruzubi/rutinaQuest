...
export function useGamificacion() {
  ...
  const cargarEstado = useCallback(async () => {
    ...
    if (guardado) {
      ...
      if (guardado.fechaHoy !== hoyStr) {
        ...
        // Resetear SIEMPRE la penalización cuando es día nuevo
        // La fechaPenalizacion guarda el día en que se aplicó (ayer o antes)
        // Al ser día nuevo, siempre hay que resetear para que pueda aplicarse hoy si toca
      ...
      return new Promise((resolve) => {
        setEstado((prev) => {
          ...
          const nuevoEstado: EstadoPersistido = {
            ...
            ultimaFecha: hoyStr,
            tareasCompletasHoy:
              prev.fechaHoy === hoyStr ? prev.tareasCompletasHoy + 1 : 1,
            fechaHoy: hoyStr,
          };
          persist(nuevoEstado);
          resolve({
            pts,
            nuevoEstado: nuevoEstado as unknown as EstadoGamificacion,
          });
          return nuevoEstado;
        });
      });
  ...
  const tareaPerdida = useCallback(async () => {
    setEstado((prev) => {
      const noTareasHoy =
        prev.tareasCompletasHoy === 0 || prev.fechaHoy !== hoyAppStr();
      const nuevoEstado: EstadoPersistido = {
        ...prev,
        totalHecho: noTareasHoy
          ? Math.max(0, prev.totalHecho - 1)
          : prev.totalHecho,
  ...
  const penalizarFinDia = useCallback(
    (
      _tareasNoHechas: number,
      _tareasHechasHoy = 0,
    ): Promise<ResultadoPenalizacion> => {
      return new Promise((resolve) => {
        setEstado((prev) => {
          ...
          const nuevasEstrellas = aplicarPenalizacion(
            prev.estrellas ?? 0,
            penalizacion,
          );
          const hoyStr = hoyAppStr();
  ...
  const forzarEstrellas = useCallback(
    async (estrellas: number) => {
      setEstado((prev) => {
        const next: EstadoPersistido = {
          ...prev,
          estrellas,
          totalHecho: estrellas,
        };
        persist(next);
        return next;
      });
